...
Vue.component('log-reg-btn', {
    ...
    methods: {
      scroll: function() {
        ...
        window.addEventListener('scroll', function () {
          ...
          // Load more games
          if ($(window).scrollTop() + $(window).height() == $(document).height() ) { //&& that.games_list > 10
            that.getGames();
          }
        })
      },
  ...
  function hasDuplicates(array) {
    var valueArr = array.map(function(game){ return game.id });
    return valueArr.some(function(game, idx){
      return valueArr.indexOf(game) != idx
    });
  }
